...
public class SwipeMenuLayout extends FrameLayout {
    ...
    onSwipeMenuListener onSwipeMenuListener;
    ...
    public void computeScroll() {
        if (state == STATE_OPEN) {
            if (mOpenScroller.computeScrollOffset()) {
                ...
                postInvalidate();
                if (mOpenScroller.getCurrX() == mBaseX)//全部打开
                    if (onSwipeMenuListener != null)
                        onSwipeMenuListener.onOpen(position, index);
            }
        } else {
            if (mCloseScroller.computeScrollOffset()) {
                int dix = mBaseX - mCloseScroller.getCurrX();
                swipe(dix);
                postInvalidate();
                if (dix == 0)//完全关闭
                    if (onSwipeMenuListener != null)
                        onSwipeMenuListener.onClose(position, index);
    ...
    public void smoothCloseMenu(int position, int index) {
        this.position = position;
        this.index = index;
        state = STATE_CLOSE;
    ...
    public void smoothOpenMenu(int position, int index) {
        this.position = position;
        this.index = index;
        state = STATE_OPEN;
    ...
    public void setOnSwipeMenuListener(SwipeMenuLayout.onSwipeMenuListener onSwipeMenuListener) {
